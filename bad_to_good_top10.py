...
# Top10 sources list
bad_by_source_df = bad_df.groupby(pd.Grouper(key='Source'))\
    .size().to_frame().reset_index()
good_by_source_df = good_df.groupby(pd.Grouper(key='Source'))\
    .size().to_frame().reset_index()

all_by_source_df = pd.concat([bad_by_source_df, good_by_source_df])\
    .groupby(['Source']).sum().reset_index()

all_by_source_sorted_df = all_by_source_df.sort_values(by=all_by_source_df.columns[1], ascending=False)

top10_sources_list = all_by_source_sorted_df.head(10)['Source'].values.tolist()
